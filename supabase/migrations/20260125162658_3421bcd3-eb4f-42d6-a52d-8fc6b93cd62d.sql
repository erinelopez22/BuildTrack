-- Fix the generate_order_number function to handle empty strings and use a better unique identifier
...
BEGIN
  ...
  IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
    -- Get the next sequence number for today using a more reliable method
    SELECT COALESCE(MAX(
      CASE 
