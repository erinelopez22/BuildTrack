  ...
  const fetchData = async () => {
    try {
      const [ordersData, projectsData] = await Promise.all([
        request<ApiOrderWithProject[]>("/api/orders?status=all"),
        request<Array<{ id: string; name: string; status: string; [key: string]: unknown }>>("/api/projects?status=active"),
      ]);
      const activeProjectOrders = (ordersData ?? []).filter((o) => o.project?.status === "active");
      setOrders(activeProjectOrders.map(mapApiOrderWithProject));
      setProjects(
        (projectsData ?? []).map((p) => ({
          id: p.id,
          name: p.name,
    ...
    try {
      await request("/api/orders", {
        ...
        body: {
          ...
          supplierName: formData.supplier_name || null,
          supplierContact: formData.supplier_contact || null,
          expectedDeliveryDate: formData.expected_delivery_date || null,
